...
import type { ReminderKind } from "../db/models";
...
export const cancelReminderCommand = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      .setName("id")
      .setDescription("Reminder ID from /timers")
      .setRequired(true),
  );
  ...
  try {
    ...
    await replyWithEmbed(
      ...
      buildReminderEmbed(
        ...
        name: "ID",
        value: `\`${formatReminderId(reminder)}\``,
      }),
    );
  } catch (error) {
...
export async function handleCancelReminderCommand(
  ...
  try {
    ...
    const embed = result.cancelled
      ...
      : buildReminderEmbed(
          "Reminder Not Found",
          "No active reminder with that ID was found in this server/private chat.",
