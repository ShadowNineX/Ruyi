...
function reminderScopeFilter(scope: ConfigScope, userId: string) {
  return {
...
class ReminderService {
  ...
  private async generateReminderMessage(
    ...
    try {
      const scope = userConfigScope(reminder.guildId, reminder.userId);
  ...
  getScope(guildId: string | null, userId: string): ConfigScope {
    return userConfigScope(guildId, userId);
  }
}
