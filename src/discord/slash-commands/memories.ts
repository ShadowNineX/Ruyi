...
async function handleRemember(
  ...
  await replyWithMemoryEmbed(
    ...
    `\`${key}\`\n${value}\n\nStored for your linked Ruyi memory.`,
...
async function handleList(
  ...
  const memories = await Memory.find(buildUserMemoryFilter(identity)).sort({
    pinned: -1,
    updatedAt: -1,
  });
  ...
  if (memories.length === 0) {
    await replyWithMemoryEmbed(
      interaction,
      "No Memories Yet",
      "I don't remember anything about your linked Ruyi identity yet.",
  ...
  await interaction.reply({
    embeds: [
      buildMemoryEmbed(
        `Memories About ${identity.username}`,
        description,
        MEMORY_COLORS.neutral,
...
export async function handleMemoriesCommand(
  ...
  const identity = buildDiscordUserIdentity(userId, username);
