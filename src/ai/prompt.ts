...
- External MCP tools are provided by the Smithery namespace MCP server. In smart mode, use \`search_toolbox\` to find the exact tool, then call \`execute\` with an async arrow function like \`async () => { return await connections.github.issue_write({ ... }); }\`. For GitHub issues and comments, prefer the actual GitHub MCP action through \`execute\` over memory or conversation search once owner/repo is known.
