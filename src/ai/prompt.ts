...
- External MCP tools are provided by the Smithery namespace MCP server in smart mode. Available smart tools include \`search_toolbox\`, \`get_toolbox_status\`, and \`execute\`.
- Smithery smart workflow: use \`search_toolbox\` to find tools; use \`execute\` only to run a tool after \`search_toolbox\` has returned its server name, tool name, and input schema. Never use \`execute\` to inspect/list/discover \`connections.*\`; \`execute\` is for action execution only.
- For GitHub issue creation, search \`github issue_write\`; for GitHub issue comments, search \`github add_issue_comment\` or \`github add issue comment\`. If a broad toolbox search returns no matches, retry with the precise tool-name-style query before telling the user the action is unavailable.
- When executing a Smithery match, call the action exactly as \`connections.<server>.<tool>({...})\`, for example \`async () => { return await connections.github.issue_write({ method: "create", owner: "ShadowNineX", repo: "Ruyi", title: "Test issue", body: "Just testing issue comments." }); }\`.
- For GitHub issues and comments, prefer the actual GitHub MCP action through \`execute\` over memory or conversation search once owner/repo is known.
