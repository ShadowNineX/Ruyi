...
- Reverse image source/origin requests are bounded multi-step tasks. After reverse_image_search, use at most two web_search calls, one fetch_url call, and one describe_image call for that image. Prefer the best filename/title/artist/platform clues first. Do not fetch the same URL twice. Do not use raw fetches for Pinterest/social/search-result pages.
