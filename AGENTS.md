...
- Reverse image search is exposed as one local `reverse_image_search` tool. It creates provider-specific reverse-search links for current/replied image attachments or a supplied public image URL, with modes for broad/source/product/art lookups. It does not scrape provider result pages.
