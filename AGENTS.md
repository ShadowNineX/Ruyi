...
Ruyi is a Nine Sols themed AI companion with a Discord transport, built on Bun + TypeScript ESM. It uses the OpenAI Agents SDK for LLM chat with function calling, MongoDB for persistence, and MCP servers for external tools.
...
1. [src/main.ts](src/main.ts) — boots DB ([src/db/index.ts](src/db/index.ts)), config + memory caches, MCP health check, then starts Discord ([src/discord/bot.ts](src/discord/bot.ts)). Registers SIGINT/SIGTERM + `unhandledRejection` / `uncaughtException` safety nets.
...
3. [src/ai/](src/ai/) — split package, re-exported via [src/ai/index.ts](src/ai/index.ts):
   ...
   - `session.ts` — `sessionManager` keyed by Discord conversation keys: `discord:<channelId>`. Sessions persist to `DiscordAgentSession`.
   - `chat.ts` — `chatService.chat()`. Returns `string | null`; **throws on error** so [src/discord/bot.ts](src/discord/bot.ts) can show a meaningful Discord message via `getErrorMessage()` in [src/discord/utils/messages.ts](src/discord/utils/messages.ts). `null` means "model returned empty" only.
   - `context.ts` — builds Discord context from channel history, reply context, temporal context, scoped config, and code-derived user memories.
   - `prompt.ts` — Ruyi's personality prompt plus shared tool-usage hints. Keep acting/cadence in the personality prompt and tool behavior in shared instructions. Wraps everything in XML-like `<context>` / `<instructions>` blocks.
...
4. [src/discord/utils/chat-session.ts](src/discord/utils/chat-session.ts) — `ChatSession` owns the typing interval and temporary tool-call embed. Normal thinking/generation uses Discord typing only; embeds are reserved for tool calls and permission prompts.
...
- Shared tools live in [src/tools/](src/tools/); Discord-only tools live in [src/discord/tools/](src/discord/tools/). They are registered centrally through [src/tools/index.ts](src/tools/index.ts).
...
- Pattern: build directly with `tool({ name, description, parameters: z.object({...}), execute })` from `@openai/agents`. Discord context flows through `runWithToolContext()` + `toolContextManager.get()`. Discord turns bind channel/guild/message/referencedMessage in [src/discord/bot.ts](src/discord/bot.ts). Tools must guard against unavailable context and return structured error objects (not throw). Use the SDK's `needsApproval` option for sensitive tools.
...
- Memory tools derive `personId` from code. The configured Discord owner user ID maps to the shared `owner` person; other Discord users get isolated `discord:<id>` memory. The AI must never provide scope IDs, guild IDs, DM IDs, or user IDs as memory tool arguments. Memory values are truncated to `MEMORY_VALUE_MAX_LEN` and evicted past `USER_MEMORY_CAP`. Ruyi stores explicit memories through the main agent's `memory_store` tool and also runs semantic c.ai-style background extraction after enough user messages. See [src/tools/memory.ts](src/tools/memory.ts), [src/utils/user-identity.ts](src/utils/user-identity.ts), and [src/ai/extraction.ts](src/ai/extraction.ts).
- `search_conversation` is the single fuzzy/exact active-conversation lookup tool. It searches scoped Discord archives only. Discord defaults to the current channel; searching another channel requires verifying it belongs to the current guild, and broad server search may only include readable guild text channels. Private chats may search only their current DM channel.

## Persistence

- Mongoose models in [src/db/models/](src/db/models/): shared `Config` and `Memory`; Discord `DiscordConversation` and `DiscordAgentSession`; Discord-only `Reminder` and `SmitheryConnection`.
...
Ruyi speaks with authentic Nine Sols cadence (formal, deferential — "your humble servant", "please forgive my apprehension"). Keep persona/acting/cadence separate from shared tool instructions in [src/ai/prompt.ts](src/ai/prompt.ts), and keep shared tool hints in sync when adding tools. Ruyi uses English by default, no speaker prefixes, embraces being an AI character, and must use `memory_recall` / `memory_store` for personal facts.
...
- When adding a tool: append to the registry in [src/tools/index.ts](src/tools/index.ts), mark self-responding/external-service if applicable, and ensure the tool only accesses Discord context via `toolContextManager.get()`.
