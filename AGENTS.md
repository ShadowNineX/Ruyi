...
- Slash commands registered at startup from [src/slash-commands/](src/slash-commands/): `/prefix`, `/model`, `/search-provider`, `/credits`, `/smithery`, `/memories`. Add new ones to the `slashCommands` array and the `handleSlashCommand` switch in [src/slash-commands/index.ts](src/slash-commands/index.ts).
