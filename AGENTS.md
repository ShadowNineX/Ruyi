...
- Reverse image search is exposed as one local `reverse_image_search` tool. It creates provider-specific reverse-search links and recommended follow-up `web_search` calls for current/replied image attachments or a supplied public image URL, with modes for broad/source/product/art lookups. Keep the investigation in the main Agents loop: call `reverse_image_search`, then continue with `web_search` / `fetch_url` as needed before answering. Do not hide a second OpenAI chat loop inside the tool.
