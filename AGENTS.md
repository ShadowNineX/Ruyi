...
- MCP-backed tools live in [src/mcp/](src/mcp/). Smithery Connect is configured with `SMITHERY_API_KEY` + `SMITHERY_NAMESPACE`; `/smithery` creates hosted Smithery setup links and stores connection IDs/statuses. Runtime MCP is passed to the OpenAI Agents SDK via `MCPServerStreamableHttp` in [src/mcp/servers.ts](src/mcp/servers.ts), using short-lived Smithery service tokens minted by [src/mcp/smithery-api.ts](src/mcp/smithery-api.ts). `SMITHERY_MCP_MODE=smart` uses Smithery's smart namespace endpoint (`https://mcp.smithery.run/{namespace}?mode=smart`); `direct` omits the mode query.
